...
int check_buffer( const char *buf){
	...
	int minus_count = 0;				/** 음수 기호 카운트 변수 */
	...
	for( ; loop_index < buf_size; loop_index++){
		...
		if( buf[loop_index] == '-'){ /** 해당 문자가 '-'일 경우, 음수를 뜻하므로 계속 진행 */
			minus_count++;
...
int input_number( int *num){
	...
	char buf[MAX_NUMS+1];		/** 입력 받을 문자열을 담을 변수 */

	printf("\t| @ 입력 : ");
	if( fgets( buf, MAX_NUMS+2, stdin) == NULL){ /** 어떠한 입력도 받지 못하거나 오류가 발생하면 NULL 을 반환한다. */
...
//	while( getchar() != '\n');
	...
	switch( return_value){
		...
		case FAIL: /** 문자열을 입력 받은 경우 */
			...
			else if( strncmp( buf, finish_code, MAX_NUMS) == EQUAL){ /** 프로그램 종료 코드(문자열)를 입력 받은 경우 */
				return_value = FINISH;
			}
...
int input_numbers( input_data_t *odd){
	int return_value_nums = SUCCESS; /** input_numbers 반환값 변수 */
	int return_value_num = SUCCESS; /** input_number 반환값 변수 */
...
void print_odd_numbers( input_data_t *odd){
	...
	printf("\n");
	printf("\t| @ 총 입력 개수\t: %d\n", odd->size); /** odd->size 는 현재 배열의 마지막 인덱스값이므로 개수를 구하기 위해서 1 을 더해준다. */
	printf("\t| @ (순서)\t\t: (입력한 정수)\n");
...
int main(){
	...
	input_data_t *odd = ( input_data_t*)malloc(sizeof( input_data_t)); /** 정수를 입력 받을 구조체를 할당해서 그 주소를 가지고 있는 구조체 포인터 변수 */
	if( odd == NULL){
	...
	while( 1){
		return_value = input_numbers( odd); /** 입력 진행 */
		switch( return_value){
			case FAIL:
